...
> - 2026-09-21：使用者決定**不做多房間**，改把唯一的房間放大（22×16 → 28×20，`31637b9`）；
>   並確認先做**語言交換活動模式**。後端（`f7311c3`：`internal/event` 輪次狀態機 + 主持人指令 +
>   `HOST_KEY` 授權）與前端（計時器/話題卡/換輪橫幅/主持面板/黑椅自動換桌）都完成，全綠，
>   另外跑了一次真實 process 煙霧測試確認 server 的 tick 迴圈真的會準時換輪。
>   **前端沒用瀏覽器實測——這次不需要麥克風就能測**，步驟見 §「Phase 2 現況（2026-09-21）」。
>   下一步候選：狀態/emote（小）、navmesh、牆上實體 TIMER 看板。
>   **收工檢查點（2026-09-21）**：`5595b97` 已 push，兩條 CI 綠燈，工作區乾淨。
>   **明天第一件事**：請使用者測語言交換活動（不需麥克風）——`realtime/.env` 加 `HOST_KEY=xxx`
>   （目前還沒有）→ 重啟 realtime → 開兩個分頁各坐一張黑椅（如桌 1、桌 2）→ 右下角 🎓 輸入金鑰、
>   開 30 秒 × 3 輪 → 看換輪時是否互換桌、橫幅/倒數/話題卡顯示是否正常。回報後再決定修 bug
>   或往下做（狀態/emote → navmesh → 牆上 TIMER 看板）。語音/視訊驗收清單仍等能測麥克風的環境。
> - 2026-09-22：使用者實測語言交換活動，**功能正常**，但回饋設計不對：不該強制黑椅的人移到
>   下一桌，應該讓參加者自己決定要不要換、什麼時候換。這其實是照回原本 PLAN §八.1 的字面
>   意思（「到點**提示**換桌」，本來就只講提示，是先前實作時自己多做了「自動走」）。
>   修正（`c437bf9`）：`Game.onRoundAdvanced` 拿掉 `standUp()`+`walkToSeat()`，只把建議桌號
>   交給橫幅顯示（「想認識新夥伴？可以移到桌 N（自行決定）」），`rotation.ts` 的純函式邏輯不變。
>   README 用語同步更新。前端全綠，push 後 CI 綠燈。
>   **收工檢查點（2026-09-22）**：`c437bf9` 已 push，工作區乾淨，遠端同步確認過。
...
#### Phase 2 現況（2026-09-21）—— 語言交換活動模式

使用者確認從這個開始（多房間已砍，見上）。這是產品核心：主持人開一場 N 輪的活動，每輪有計時器和
一個話題，**每輪結束坐黑椅（rotator）的人自動換到下一桌**（16 → 1 環繞），白椅（anchor）不動。

**已完成**
- `realtime/internal/event`（新）：`Session` **純狀態機**——不讀時鐘、不開 goroutine，每個方法
  收 `now`；Room 既有的 tick 驅動 `Tick()`，測試用假時間快轉（不用真的等 15 分鐘）。
  Idle → Running → Finished（停留 30 秒讓晚進場的人也看到結語）→ Idle。
  `Start`（1–16 輪、每輪 10–3600 秒；話題比輪數少會循環，沒給就用內建 10 個）/ `Next` /
  `Extend`（移動截止時間，不改名義每輪長度）/ `End`。**不知道座位/桌子**：誰移去哪由
  client 從輪次數字推導。
- `protocol`：`SessionState`、`ServerMsg.Now`（server 時間，client 用來修正時鐘偏差）、
  主持人指令欄位、`hostResult`。
- `game`：`Room` 持有 `Session`，tick 時輪次變化就廣播；**每個新加入的 client 都會收到目前狀態**
  （連 idle 也送，清掉重連前殘留的舊狀態）。`handleHost`：用共用金鑰 `HOST_KEY` 授權
  （常數時間比對；**沒設就完全停用**，公開部署的安全預設——目前沒帳號可以掛「主持人」角色）；
  同一條連線猜錯 5 次就擋（門檻性質，不是真正的 rate limit，重連即重置）；先回覆
  `hostResult` 再廣播新狀態。
- 前端：`engine/rotation.ts`（純函式：`nextTable` 環繞、`rotationTarget`（黑椅 → 下一桌黑椅，
  白椅/沒坐 → null）、`remainingMs` 用 server 時間差修正倒數、`formatClock`）；
  `Game.onRoundAdvanced`：**只有「進行中的活動輪次往前走」才會觸發提示**，剛加入/重連到同一輪
  不會被打擾；缺了好幾輪（斷線）建議的桌號會一次跳好幾桌。**2026-09-22 改成純提示、不自動移動**
  （見下方同日補充）——只算出建議桌號交給 UI 顯示，實際要不要換由玩家自己點椅子。
  UI：`SessionPanel`（第 N/M 輪 + 倒數 + 話題卡，剩 30 秒變橘色，結束時顯示結語）、
  `RoundNotice`（換輪大橫幅：黑椅的人會看到「可以移到桌 N」的建議）、`HostConsole`（右下角 🎓，
  金鑰存自己瀏覽器 localStorage、開始表單 / 活動中的下一輪·+1·+5 分鐘·結束）。
- 順手：`.env.example` 加 `HOST_KEY`，README 補主持人使用說明。

**驗證**
- Go：`event` 15 個測試（假時間）+ `game` 8 個真 WebSocket 整合測試（廣播給所有人、
  next/extend/end、金鑰錯誤/停用/猜錯上限、指令錯誤、晚進場者）。
- 前端：`rotation` 16 個 vitest 測試（環繞、多桌跳、時鐘偏差、進位）。
- **真實 process 煙霧測試**（單元測試用假時間，沒走過真的 tick 迴圈，所以另外跑）：編譯 server、
  兩條 WebSocket、10 秒 × 2 輪——第 2 輪在 11.3s（開始後 ~10s）由 server 自己換，倒數重新給滿
  10000ms，21.4s 結束，兩邊都收到。
- 過程中抓到並修掉的 bug：`resetLocked` 寫成 `*s = Session{}` 會連持有中的 mutex 一起覆寫，
  之後 deferred `Unlock` 會 panic——測試前就看出來了，改成逐欄位重設。
...
3. ~~「提示 vs 自動走過去」目前固定是自動走~~ ← ✅ **2026-09-22 改掉**：使用者實測後回饋不該強制
   ...
   時自己多做了「自動走」，這次改回符合原文。
4. 沒有對「一直沒人坐白椅／黑椅」的桌子做特別處理（白椅的人不動、黑椅照轉，本來就跟對面
   有沒有人無關，符合 PLAN 的規則）。

#### Phase 2 現況（2026-09-22 補）—— 換桌從「自動走」改成「純提示」

使用者實測回報：自動幫人移到下一桌不對，應該讓參加者自己決定要不要換、什麼時候換。
`Game.onRoundAdvanced` 移除 `standUp()`+`walkToSeat()` 的強制移動，只保留算出建議桌號給
`RoundNotice` 顯示（文字改成「想認識新夥伴？可以移到桌 N（自行決定）」）；`rotation.ts` 的
`rotationTarget` 純函式不變，只是呼叫端不再拿它的結果去移動玩家，改拿去顯示建議。

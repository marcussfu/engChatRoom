...
- 前端：`engine/rotation.ts`（純函式：`nextTable` 環繞、`rotationTarget`（黑椅 → 下一桌黑椅，
  ...
  `Game.onRoundAdvanced`：**只有「進行中的活動輪次往前走」才會觸發換桌**，剛加入/重連到同一輪
  不會被推來推去；黑椅的人 `standUp()` + 既有的 `walkToSeat()` 自己走過去（**刻意不檢查目標座位
  是否有人**——所有黑椅的人同時離開，目標座位正好同時空出）；缺了好幾輪（斷線）會一次轉多桌。
  UI：`SessionPanel`（第 N/M 輪 + 倒數 + 話題卡，剩 30 秒變橘色，結束時顯示結語）、
  `RoundNotice`（換輪大橫幅：黑椅的人會看到「請移到桌 N」）、`HostConsole`（右下角 🎓，
...
**尚未做 / 已知限制**
1. **完全沒用瀏覽器實測前端**（自動換桌走路、橫幅、主持面板的實際觀感）。這塊**不需要麥克風**，
   使用者可以直接測：`realtime/.env` 加 `HOST_KEY=xxx` → 重啟 → 右下角 🎓 開始一場 30 秒×3 輪的
   活動 → 兩個分頁各坐一張黑椅（例如桌 1、桌 2），看換輪時是不是互相換桌。
2. **牆上的實體 TIMER 看板**沒做——目前只有 HUD 面板。要做的話用 Babylon `DynamicTexture`
   把文字畫到牆面平面上（純 canvas 貼圖，沒有 iframe 問題），順便跟 render-on-demand 協調
   （活動中每秒需要重繪一次）。
3. 「提示 vs 自動走過去」目前固定是**自動走**（PLAN §八.1 說可設定「只提示」）；沒做切換。
